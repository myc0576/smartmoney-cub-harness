...
| --- | --- | --- | --- |
| [TradingAgents](integrations/tradingagents.md) | `documented-adapter` | Optional upstream research provider. Its Markdown reports may be normalized into `external_analysis_artifact` review evidence. | No default runtime dependency, no copied core code, no broker execution, no order routing, and no automatic champion mutation. |
